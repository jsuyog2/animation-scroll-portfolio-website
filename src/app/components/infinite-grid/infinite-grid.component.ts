...
export class InfiniteGridComponent {
  ...
  intialGrid() {
    ...
    this.createMask();
    this.lastCenteredElem = document.querySelectorAll(`.${this.divClass}`)[
      (this.rowMidIndex - 1) * this.contentNum + this.contentMidIndex
    ];
  ...
  initializeCardClickEvent(drag: any) {
    ...
    gridContainer.touchcancel = openCard;
  ...
  addValues(data: any) {
    ...
    const link: any = document.querySelector(
      '.content-body h3.title #external-link'
    );
    const linkText: any = document.querySelector(
      '.content-body h3.title #external-link-text'
    );
    const duration: any = document.querySelector(
      '.content-body p.duration var'
    );
    const description: any = document.querySelector(
      '.content-body ul.description'
    );
    const responsibility: any = document.querySelector(
      '.content-body ul.responsibility'
    );
    const technologies: any = document.querySelector(
      '.content-body div.technologies'
    );
    ...
    const descriptionData = data.description
      ?.map((val: any) => `<li>${val}</li>`)
      .join('');
    description.innerHTML = descriptionData;

    const responsibilityData =
      data.responsibility?.map((val: any) => `<li>${val}</li>`).join('') || '';
    responsibility.innerHTML = responsibilityData;

    const technologiesData = data.tech
      ?.map(
        (val: any) =>
          `<div class="tech d-flex flex-column justify-content-center align-items-center p-3"><img class="w-50" src="${val.icon}"/><label class="fw-medium text-center">${val.title}</label></div>`
      )
      .join('');
  ...
  createImageGrid() {
    ...
    for (let y = 0; y < this.rowNum; y++) {
      ...
      for (let x = 0; x < maxLength; x++) {
        ...
        div.innerHTML = `
            <h4>${title}</h4>
            <div id="content${y}${x}" data-row="${this.tags[y]}" data-index="${
          x % items.length
        }" class="p-card p-component shadow">
      ...
      this.contentRep.push(
        gsap.utils.toArray(row.querySelectorAll(`.${this.divClass}`))
      );
    }
  ...
  createMask() {
    ...
    mask.className = 'mask';
    let container: any = document.querySelector(
      `#${this.containerId}`
    )?.parentElement;
  ...
  createDraggable() {
    ...
    if (this.useInertia) {
      ...
      if (this.useCenterGrid) {
        options.onThrowComplete = () => {
          this.centerGrid();
        };
      }
    } else if (this.useCenterGrid) {
      // No inertia
      options.onDragEnd = () => {
        this.centerGrid();
      };
  ...
  updateCenterElem() {
    ...
    elems.forEach((elem) => {
      if (
        elem.matches(`.${this.divClass}`) &&
        !this.lastCenteredElem.isSameNode(elem)
      ) {
    ...
    if (rowIndex < this.rowMidIndex) {
      for (let i = rowIndex; i < this.rowMidIndex; i++) {
        ...
        if (this.rowArray.length % 2 === 1) {
          ...
          gsap.set(this.rowArray[this.rowArray.length - 1], {
            y: rowY - this.gutter - this.boxHeight,
          });
        }
      ...
      for (let rowNum = 0; rowNum < this.rows.length; rowNum++) {
        ...
        for (let i = imgIndex; i < this.contentMidIndex; i++) {
          ...
          gsap.set(row[row.length - 1], {
            x: imgX - this.gutter - this.boxWidth,
          });
  ...
  resize() {
    ...
    this.horizOffset =
      -(this.contentMidIndex * this.horizSpacing + this.boxWidth / 2) +
      this.winMidX;
    this.vertOffset =
      -(this.rowMidIndex * this.vertSpacing + this.boxHeight / 2) +
      this.winMidY;
    ...
    this.rows.forEach((row: any, i: any) => {
      ...
      gsap.set(row, {
        ...
        x: () => {
          if (i % 2 === 0) return this.horizOffset;
          else return this.horizOffset - this.boxWidth / 2;
        },
        y: () => {
